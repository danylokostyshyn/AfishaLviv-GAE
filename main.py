...
class EventInfoPage(webapp2.RequestHandler):
    ...
    def getEventExtendedInfo(self, event_url):
        ...
        try:
            ...
            current_text = (utils.Decoder.decode_unicode_references(tmp))
...
class PhotoEvents(webapp2.RequestHandler):
    def get(self):
        self.response.headers['Content-Type'] = 'application/json'

        try:
            page_number = int(self.request.get("p"))
            offset = (page_number -1) * 16
            url = "http://afishalviv.net/page/ua/photos/?photos_p=%d" % offset
        except: 
            url = "http://afishalviv.net/page/ua/photos/"

        json = memcache.get(url)
        if json is not None:
            self.response.out.write(json)
        else: 
            json = self.getPhotoEvents(url)
            memcache.add(url, json, 60*60*24)
            self.response.out.write(json)

    def getPhotoEvents(self, url):
        logging.info("photo events url: %s", url)

        page = urllib.urlopen(url)
        root = html.parse(page).getroot()

        results = []
        items = root.xpath("//div[@class='photoSearch']/ul/li")
        for item in items:
            title = item.xpath("h2")[0].text_content()
            link = item.xpath("h2/a/@href")[0]
            thumb_url = item.xpath("div//img/@src")[0]
            results.append( {"title":title, "url":link, "thumb_url":thumb_url} );

        return simplejson.dumps(results, ensure_ascii=False, indent=4, sort_keys=True);
